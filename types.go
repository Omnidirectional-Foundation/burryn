...
func (c *Checker) declareBuiltins() {
	...
	decl("byte_chr", mono(fn(tStr, tInt)))
	decl("ord", mono(fn(tInt, tStr)))
	decl("yield", mono(fn(tUnit)))
	decl("tcp_listen", mono(fn(res(tInt, tStr), tStr, tInt)))
	decl("tcp_accept", mono(fn(res(tInt, tStr), tInt)))
	decl("tcp_dial", mono(fn(res(tInt, tStr), tStr, tInt)))
	decl("net_read", mono(fn(res(tStr, tStr), tInt, tInt)))
	decl("net_write", mono(fn(res(tUnit, tStr), tInt, tStr)))
	decl("net_close", mono(fn(tUnit, tInt)))
	decl("net_nb", mono(fn(res(tStr, tStr), tInt, tInt, tStr, tInt)))
	decl("read_stdin", mono(fn(tStr, tInt)))
	decl("stdin_nb", mono(fn(res(tStr, tStr), tInt)))
