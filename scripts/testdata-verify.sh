...
echo "--- C 后端双级 fixpoint ---"
GEN0=/tmp/td-gen0
GEN1=/tmp/td-gen1
if "$BUR" build compiler -o "$GEN0" 2>/dev/null && "$GEN0" build compiler -o "$GEN1" 2>/dev/null && cmp "$GEN0" "$GEN1"; then
    echo "PASS fixpoint (gen0 == gen1)"
else
    echo "FAIL fixpoint"
    fails=$((fails + 1))
fi
rm -f "$GEN0" "$GEN1" program.c
