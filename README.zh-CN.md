...
- **网络端点**：未来依赖拉取（S6）将通过网络执行 `git clone`；当前版本仅解析本地包。
...
S1–S5 阶段已完成。
覆盖范围包括语义内核、C 后端、模块、map、`select`/`close`、`mut` 参数，以及移除 Go 宿主后的完全自举编译器。
两个后端对整门语言（含并发）都能产生逐字节一致的输出。

仍缺失：records/structs（可用单变体枚举建模乘积类型）、`defer`、字符串插值、第三方依赖拉取（目前仅解析本地包）。
