...
- **崩溃可诊断性对齐 C 的覆盖面**（三目标）：C 运行时的纤程切换走 `ucontext`/Win32 Fiber API（`runtime/burrt_impl.c` 的 `getcontext`/`bur_switch_to_sched`），这类系统级协程切换本身对栈回溯就是不透明的——所以 PE/Mach-O/ELF 已落地的"只覆盖用户函数，纤程切换点（yield/调度器恢复）不覆盖"这条边界，其实已经对齐 C 的真实能力，不是缺口。(a) **ELF `.eh_frame`（DWARF CFI）** ✅ 已完成（2026-09-14）：单份共享 CIE + 每函数一条 FDE，覆盖范围与已交付的 PE `.pdata`/Mach-O `__unwind_info` 对齐；用真实工具链逐字节核对编码，再用编译器实际产出的 ELF 在 gdb 下跑深度非尾递归到真实栈溢出崩溃验证回溯正确，详见 `architecture.md` §5.19。**剩余的真正缺口**是：GC/调度器里*不做*纤程切换、只是普通 `call`/`ret` 的内部子程序（`gc_record` 9-push、`mark_addr` 7-push、`chan_schedule`/`wake_waiters`/`push_queue`/`remove_waiter`/`waitset_add`/`timer_add`）三个格式都还没纳入 unwind 覆盖，而 C 编译同等函数会自动拿到完整栈回溯信息——这些是普通序言，补齐是已交付机制的直接扩展，每种序言形态多算一份编码，不是新架构。
