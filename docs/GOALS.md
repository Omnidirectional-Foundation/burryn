...
**当前推进顺序(2026-07-17；deep-mut、S6.6 核心、缓存包 import、S7.8 与 seed 重新接生修复已完成)**：S6.1 接口缓存 → 子包测试发现 + `bur mod tidy` 按 import 增删 require → CI `bur dev embed-std` regen+cmp → 诊断/DX 批(S6.5) → **S6 收尾 = v0.3 发布 + seed 定基** → S7。
