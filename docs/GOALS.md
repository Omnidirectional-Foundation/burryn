...
- **`mut` 为深语义、绑定级纪律(owner 2026-07-10 修订)**：经由 `let` 绑定名不可修改其值(含容器内容)；push/元素修改要求 `mut`。
  ...
  **只对堆类型(list/map 及含其的类型)生效**：int/float/bool/str/unit 为拷贝语义、无别名危害，豁免；**chan 整体豁免、不论元素类型**(owner 2026-07-12 定：send/recv/close 在现行纪律里本就不要求 mut，chan 别名是 CSP 语义本体，如 examples/concurrency/sieve.bur 的链式重绑)；if/match 作来源时递归看各臂尾表达式，皆新鲜则整体新鲜；来源类型未解时延迟判定(复用 S6.8 的组尾 flush 机制)。
...
**S7.3 落地状态(2026-07-19)**：match 臂语法为 `pattern if guard => body`；guard 在 pattern 绑定作用域内求值且必须为 `bool`；带 guard 的臂不计入穷尽覆盖。compiler 在 pattern 成功后发条件跳转，guard 为假时关闭捕获并清理本臂局部变量后继续匹配；VM/native parity 与 gen2 == gen3 已验证。

**S7.2 落地状态(2026-07-19)**：lexer 新增 `|>` token(先于 `||` 匹配)；parser 以最低优先级左结合解析，RHS 走专用 pipe target 产生式(裸名、`pkg.f`，可带实参)，`x |> (f())` 报 E1101；AST 保留 `Pipe` 节点，checker 前一趟 lowering 把 `Pipe(lhs, target, args)` 降为 `Call(target, [lhs] + args)`，checker/compiler 零新增语义；formatter 逐字重建 `|>` 表层并按 span 区分 `x |> f` 与 `x |> f()`；`dev parse`/`dev fmt` 保持原始 Pipe AST。gen1 == gen2 == gen3 与 seed 三段链定点已验证。

**S7.6 落地状态(2026-07-20)**：`defer { ... }` 语句挂包围函数，块编译为零参闭包、由新 opcode 压入当前调用帧的 defer 栈，函数退出时 LIFO 执行——`return`、末尾表达式与 `?` 早退（op_try 的 Err 路径）均触发，trap/死锁维持进程级 abort 不执行。脚本与包文件顶层的 defer 挂各自合成入口(`<script>`/`<init>`)，随程序体收尾执行。VM 侧为每帧维护 defer 列表与退出值 stash，返回指令原地重入直至 defer 耗尽；C 侧为每 fiber 维护 defer 数组，含 defer 的函数以入口水位(dbase)划界、两条退出路径先跑 `bur_run_defers` 再弹出退出值（退出值全程留栈保持 GC 可达），两运行时 defer 闭包均纳入 GC 根。checker 按闭包体检查块（块内 `return`/`?` 只退出块本身），弃值沿用 unused_must_use：块结果为未处理的 Result/Option 报错。缺块报 E1101 前置为专用 E1119；formatter 重建 `defer { ... }` 表层并保留注释；`bur dis` 增 DEFER 名。VM/native parity（含 fiber、trap trace）、examples golden（新增 cleanup）、gen1 == gen2 == gen3 与 seed 三段链定点（t2 == t3 且与 gen2 同 sha）均已验证。

**S7.7 落地状态(2026-07-22)**：六个 TCP native（`tcp_listen`/`tcp_accept`/`tcp_dial`/`net_read`/`net_write`/`net_close`）全部单态、fiber 级阻塞。C 运行时以非阻塞 socket + `bur_wait_current_fd` 注册 fd 到通用 wait set（2a 重构统一了 exec/timer/socket 入口），调度器 `bur_wait_poll` 一次 poll 唤醒所有就绪 fiber；`net_close` 取消同 fd 的 blocked fibers 防止 close 后 fd reuse 误唤醒。VM 侧新增 `net_nb(op, h, data, max)` 非阻塞 native（op 0=accept / 1=read / 2=write），EAGAIN 返回 `Err("__eagain")` 哨兵；`do_native` 的 tcp_accept/net_read/net_write 臂首次尝试后若 EAGAIN 则以 `fs_blocked_net` 状态 park guest fiber，调度器 `wait_poll_net` 轮询重试并经 `fb_iohas`/`fb_iores` 投递结果（与 exec 的 park-and-deliver 同构）；net_write 实现 write-all，park 跨周期追踪剩余偏移。tcp_listen/tcp_dial/net_close 直调宿主同步版（DNS getaddrinfo 同步阻塞整个调度器，标注为已知限制）。`net_close` 无效/重复句柄 = trap。附带最小 `std/net` 包装包（`read_all` 读到 EOF、`write_line` 写带换行），随 `std_embed` 分发。三段链约束触发两次定基：seed-base-2（decl + C 实现，vm.bur 不消费）与 seed-base-3（追加 `net_nb`）；Go seed 移植了 SCC 依赖序推导修复多态前向引用钉死（quirk #9）。测试 = loopback 双 fiber 端到端 golden + 错误路径（端口占用/连接拒绝/读 EOF）golden + 无效句柄 trap；VM/native parity 与 `BUR_DETERMINISTIC=1` 输出稳定均已验证；CI seed-base-3 三段链全绿。v1 不做 UDP/unix socket/TLS。

**S7.5 落地状态(2026-07-19)**：`const N = <expr>` 支持包级(含 `pub`)与 block 级声明；fold 阶段在 checker 前处理字面量、const 引用、算术/比较/bool 运算与 str 拼接，包级常量跨文件按 fixpoint 求值并支持跨 package 的公开引用；声明统一降为不可变 `let`，因此沿用既有类型推导与不可重赋值规则。非可折叠表达式报 E0015，折叠期整数溢出/除零报 E0080，常量引用环报 E0391；逻辑运算保留短路语义。VM 与 C runtime 的 int-int 有序比较改为精确 `int64` 比较，避免边界值经 `double` 转换失真。formatter、公开命令、dev parity、test runner 与 interface 生成链均已接入；脚本/模块示例和错误 fixture 已覆盖，VM/native parity、gen2 == gen3 与 seed 三段链定点均已验证。
